        ...
        public function index()
        {
            ...
            foreach ($all_cells as $cell)
            {
                if ($cell->x_pos == $capital->x_pos && $cell->y_pos == $capital->y_pos)
                    array_push($visible_cells, ["type" => trans('map.capital'), "format_type" => "capital", "x_pos" => $cell->x_pos, "y_pos" => $cell->y_pos, "background-color" => "lemonchiffon", "color" => "green", "class" => "fa-star", "name" =>  $city->name, "diplomatie" => "owned", "race" => $user_race_name]);
